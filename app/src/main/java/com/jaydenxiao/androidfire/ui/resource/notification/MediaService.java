...
public class MediaService extends Service {
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        if(command==CommandClose){
            ...
            manger.cancel(Demo02Fragment.TYPE_Customer);
        }else {
    ...
    private void sendCustomerNotification(int command){
        ...
        if(command==CommandNext){
            remoteViews.setImageViewResource(R.id.btn1,R.mipmap.ic_pause_white);
        }else if(command==CommandPlay){
            if(playerStatus==StatusStop){
                remoteViews.setImageViewResource(R.id.btn1,R.mipmap.ic_pause_white);
            }else{
                remoteViews.setImageViewResource(R.id.btn1,R.mipmap.ic_play_arrow_white_18dp);
            }
        }
        ...
        manger.notify(Demo02Fragment.TYPE_Customer,notification);
    }
